...
const fecthFromAccount = ({ endpoint, options, SUCCESS_TYPE }) => dispatch => {
...
export const signup = ({ username, password }) => fecthFromAccount({
    endpoint: 'signup',
    options: {
        method: 'POST',
...
export const login = ({ username, password }) => fecthFromAccount({
    endpoint: 'login',
    options: {
        method: 'POST',
        body: JSON.stringify({ username, password }),
...
export const logout = () => fecthFromAccount({
    endpoint: 'logout',
    options: { credentials: 'include' },
    SUCCESS_TYPE: ACCOUNT.FETCH_LOGOUT_SUCCESS
});

export const fetchAuthenticated = () => fecthFromAccount({
